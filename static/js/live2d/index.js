...
const PLUGINS_LIVE2D = new (function () {
    ...
    this.initMyLive2d = function (live2d_width, live2d_height, position, minWidth) {
        ...
        $("<link>").attr({
            href: "https://live2d.fghrsh.net/assets/1.4.2/waifu.min.css",
            rel: "stylesheet",
            type: "text/css"
        }).appendTo('head');
        ...
        $.ajax({
            url: 'https://live2d.fghrsh.net/assets/1.4.2/jquery.min.js',
            dataType: "script",
            cache: true,
            async: false
        });
        ...
        $.ajax({
            url: 'https://live2d.fghrsh.net/assets/1.4.2/jquery-ui.min.js',
            dataType: "script",
            cache: true,
            async: false
        });
        $.ajax({
            url: "https://live2d.fghrsh.net/assets/1.4.2/waifu-tips.min.js",
            dataType: "script",
            cache: true,
            success: function () {
                $.ajax({
                    url: "https://live2d.fghrsh.net/assets/1.4.2/live2d.min.js",
                    dataType: "script",
                    cache: true,
                    success: function () {
                        /* 可直接修改部分参数 */
                        ...
                        initModel({
                            ...
                            "mouseover": [
                                ...
                                {
                                    "selector": ".container a[href^='#']",
                                    "text": ["要看目录 <span style=\"color:#0099cc;\">{text}</span> 么？"]
                                },
                                {
                                    "selector": ".container a[href^='/']",
                                    "text": ["要看看 <span style=\"color:#0099cc;\">{text}</span> 么？"]
                                },
                                {
                                    ...
                                    "text": ["要看看菜单 <span style=\"color:#0099cc;\">{text}</span> 么？"]
                                },
                                {
                                    "selector": "#share-bar a[href]",
                                    "text": ["好东西要和朋友一起分享 <span style=\"color:#0099cc;\">{text}</span> 么？"]
                                },
                                {
                                    "selector": ".widgets .fixed-button",
                                    "text": ["小助手 <span style=\"color:#0099cc;\">{text}</span> "]
                                },
                                {
                                    "selector": ".search",
                                    "text": ["<span style=\"color:#0099cc;\">这里可以找到你想要的东西哟{text}</span> "]
                                },

                                {"selector": ".aplayer-info", "text": ["来一起听音乐吧~~~ {text}"]},
                                {
                                    "selector": ".aplayer-list li",
                                    "text": ["听听 <span style=\"color:#0099cc;\">{text}</span> ？"]
                                },
